...
```
.kiro/
├── steering/          # Kiro エージェントへのコンテキスト注入
│   ├── process.md    # 開発プロセス原則（常時読込・編集不要）
│   ├── product.md    # プロダクト定義（常時読込・要カスタマイズ）
│   ├── tech.md       # 技術スタック・環境設定（常時読込・要カスタマイズ）
│   ├── structure.md  # リポジトリ構造（常時読込・要カスタマイズ）
│   └── skill-*.md    # 詳細手順（チャットで `#ファイル名` を入力したときのみ読込）
├── hooks/             # 定型作業の自動化（add-feature / setup-project / tasklist-check）
├── agents/            # カスタムエージェント（@implementation-validator / @doc-reviewer）
└── settings/
...
Kiro が SDD フロー（requirements → design → tasklist → **計画の承認** → 実装 → 3段検証 → 振り返り → PR）をガイドします。承認ゲートは「計画の承認」の1箇所のみで、承認後は完了まで自動で実行されます。
